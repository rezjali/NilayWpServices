...
class NSM_Admin_Handler {
    public static function ajax_test_gateway() {
        ...
        if (!current_user_can('manage_options')) {
            wp_send_json_error(nsm_get_string('error_permission_denied', 'دسترسی غیرمجاز.'));
        }

        $gateway = isset($_POST['gateway']) ? sanitize_key($_POST['gateway']) : '';
        
        if ($gateway) {
            wp_send_json_success('اتصال آزمایشی با موفقیت برقرار شد. (شبیه‌سازی شده)');
        } else {
            wp_send_json_error('درگاه مشخص نشده است.');
        }
    }
