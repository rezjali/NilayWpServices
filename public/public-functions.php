...
class NSM_Frontend {
    public static function display_single_service_page($content) {
        if (is_singular('service') && in_the_loop() && is_main_query()) {
            ...
            $service_content = '';
            // Pass the mode to each render function
            switch ($template) {
                case 'layout-2': $service_content = self::render_template_two($post_id, $template_mode); break;
                case 'layout-3': $service_content = self::render_template_three($post_id, $template_mode); break;
                case 'layout-4': $service_content = self::render_template_four($post_id, $template_mode); break;
                case 'layout-5': $service_content = self::render_template_five($post_id, $template_mode); break;
                case 'layout-6': $service_content = self::render_template_six($post_id, $template_mode); break;
                case 'layout-7': $service_content = self::render_template_seven($post_id, $template_mode); break;
                case 'layout-1': default: $service_content = self::render_template_one($post_id, $template_mode); break;
            }
    ...
    private static function render_content_section($title, $content, $apply_wpautop = true) {
        if (!empty($content)) {
            // For textarea content, we allow some basic HTML, so we use wp_kses_post
    ...
    private static function render_payment_box($post_id) {
        ...
        if ($payment_model === 'paid') {
            if ($reg_link) {
                ...
                echo '<div class="nsm-price-box"><span class="price-value">' . number_format($price) . '</span> <small>' . nsm_get_string('toman', 'تومان') . '</small></div><hr>';
                ...
                echo '<input type="tel" name="user_mobile" placeholder="' . nsm_get_string('mobile_number', 'شماره موبایل') . '" required>';
                echo '<button type="submit" class="nsm-cta-button">' . nsm_get_string('payment_and_reg', 'پرداخت و ثبت نام') . '</button>';
                echo '</form>';
            }
        } else {
            echo '<form method="POST" class="nsm-payment-form">';
            // Free registration logic here
    ...
    private static function get_meta_fields_for_sidebar($post_id, $service_type) {
        ...
        // Remove leading number and dot (e.g., "۱. ") - Supports Persian/Arabic numerals
